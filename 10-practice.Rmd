...
- What does the `Customers` term most likely refer to?
- What do you think the `SELECT` function does?
- What does `*` indicate, in combination with `SELECT`? 
- Parsing the second line that starts with `WHERE`, can you make an educated guess about what `Country` and `CustomerName` refer to?
- Can you guess (or feel free to use Google) what `LIKE 'G%'` and `LIKE 'R%'` would indicate?

<details>
<summary>Do your answers match these?</summary>
- The `Customers` term here refers to a table or dataframe. 
- The `SELECT` function selects columns from a table. 
- The `*` here indicates that we want to return all columns from the table. 
- The `Customers` table must have columns named `Country` and `CustomerName`. 
- `LIKE` is used for pattern matching. This code is matching values that start with "G" or with "R".
</details>\
...
Starting from the `Customers` table, return all columns but only rows where `Country` is equal to "Spain" __and__ where the `CustomerName` value starts with "G" or "R".
