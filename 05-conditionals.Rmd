...
Python uses indentation for code blocks.  Programming structures, such as `if`, `else`, `elif`, or `for` expects `:` at the end of each statement with the subsequent code block intented by one
```
if x == y:
  print("values are equal")
...
R makes use of curly brackets `{}` to indicate each section of an `if else` statement and while indentation is a convention for readability, it is not technically required for the code to run:
