...
for ingredient in ingredents
  write(ingredient)
```
at the top of your paper, write out the `ingredients` assignment.  On the line below that we're going to create create headers for all of the variables used within the for loop code block.  Starting on the left going right, write the term `loop iteration`, then `ingredient` in the center of the line, then the term `write` on the right of the line. 
