...
\item
  \emph{Challenge}: For the following dataframe, named \texttt{df},
\end{enumerate}
...
\tightlist
\item
  \texttt{temperature\ =\ 75\ (integer)}
\item
  \texttt{precipitation\ =\ 1.2\ (float)}
\item
  \texttt{location\ =\ California\ (string)}
\end{itemize}
...
\setcounter{enumi}{1}
\item
  The value of \texttt{cheese{[}4{]}} is \texttt{Muenster}, the fourth item in the \texttt{cheese} array.
\item
  The value of \texttt{potatoes{[}3,2{]}} is \texttt{Kennebec}, which is the value in row 3, column 2.
\item
  \texttt{df\$ingredients{[}3{]}} would return \texttt{milk}. We can guess that \texttt{df\$ingredients} means the \texttt{ingredients} column in the dataframe \texttt{df} (even though the \texttt{\$} symbol is new). From there, we can use the same indexing approach as the other questions, such that \texttt{df\$ingredients{[}3{]}} indicates the third rows down in the \texttt{ingredients} column, which is \texttt{milk}.
\end{enumerate}
...
\begin{Shaded}
\begin{Highlighting}[]
\ControlFlowTok{for}\NormalTok{ (}\VariableTok{$x} \OperatorTok{=} \DecValTok{0}\OtherTok{;} \VariableTok{$x} \OperatorTok{\textless{}=} \DecValTok{5}\OtherTok{;} \VariableTok{$x}\OperatorTok{++}\NormalTok{) \{}
  \KeywordTok{echo} \VariableTok{$x}\StringTok{";}
\StringTok{\}}
\end{Highlighting}
\end{Shaded}
