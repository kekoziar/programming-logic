...
The for loop is probably the most common type of loop. It executes a chunk of code \emph{for} a certain number of times. The common structure of most for loops is
...
at the top of the paper, write out the \texttt{ingredients} assignment, then right below that create headers for all of the variables used within the for loop code block, and add term \texttt{write} and another term \texttt{loop\ iteration}. As you were rewriting this, you may have noticed the typo \texttt{ingredents} in the for loop. The nice thing about using pseudocode is you don't need to worry about why your code isn't running if there are typos!
