...
\tightlist
\item
  What does the \texttt{Customers} term most likely refer to?
\item
  What do you think the \texttt{SELECT} function does?
\item
  What does \texttt{*} indicate, in combination with \texttt{SELECT}?
\item
  Parsing the second line that starts with \texttt{WHERE}, can you make an educated guess about what \texttt{Country} and \texttt{CustomerName} refer to?
\item
  Can you guess (or feel free to use Google) what \texttt{LIKE\ \textquotesingle{}G\%\textquotesingle{}} and \texttt{LIKE\ \textquotesingle{}R\%\textquotesingle{}} would indicate?
\end{itemize}
...
\tightlist
\item
  The \texttt{Customers} term here refers to a table or dataframe.
\item
...
\item
  The \texttt{Customers} table must have columns named \texttt{Country} and \texttt{CustomerName}.
\item
  \texttt{LIKE} is used for pattern matching. This code is matching values that start with ``G'' or with ``R''.
\end{itemize}
...
Starting from the \texttt{Customers} table, return all columns but only rows where \texttt{Country} is equal to ``Spain'' \textbf{and} where the \texttt{CustomerName} value starts with ``G'' or ``R''.
