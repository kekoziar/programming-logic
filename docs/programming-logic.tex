...
Functions will have inputs \emph{(usually)} and outputs. The function inputs are formally called ``arguments,'' and are specified by the programmer, entered within the parentheses. For cheesy mashed potatoes, this could be specifying what kind of cheese to use, or the quantity of each ingredient.

While our example is in pseudocode, luckily function calls look the same across most languages.

\begin{verbatim}
cheesy_mashed_potatoes(cheese, potatoes, milk, salt, butter)
\end{verbatim}
